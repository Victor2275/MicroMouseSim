...
public class Maze {
    ...
    public void drawMaze(Graphics g) {
        ...
        for (int i = 0; i < mazeActual.length; i++) {
            for (int j = 0; j < mazeActual[i].length; j++) {
                createSquare(i, j, mazeActual[i][j], g2D, new Color(200, 50, 50));
                createSquare(i, j, mazeFound[i][j], g2D, new Color(50, 200, 50));
            }
        }
    ...
    public void createSquare(int y, int x, Square square, Graphics g, Color color) {
        ...
        if (square.getWallDown()) {
            g2D.drawLine((50 * x + 10) / scaleFactor, (50 * y + 60) / scaleFactor, (50 * x + 60) / scaleFactor,
                    (50 * y + 60) / scaleFactor);
        }
        if (square.getWallUp()) {
            g2D.drawLine((50 * x + 10) / scaleFactor, (50 * y + 10) / scaleFactor, (50 * x + 60) / scaleFactor,
                    (50 * y + 10) / scaleFactor);
        }
        if (square.getWallLeft()) {
            g2D.drawLine((50 * x + 10) / scaleFactor, (50 * y + 10) / scaleFactor, (50 * x + 10) / scaleFactor,
                    (50 * y + 60) / scaleFactor);
        }
        if (square.getWallRight()) {
            g2D.drawLine((50 * x + 60) / scaleFactor, (50 * y + 10) / scaleFactor, (50 * x + 60) / scaleFactor,
                    (50 * y + 60) / scaleFactor);
        }
    }
    ...
    public void checkSquare(int x, int y) {
        ...
        if (x < size - 1) {
            mazeFound[y][x + 1].setWallLeft(mazeActual[y][x + 1].getWallLeft());
        }
        if (x > 0) {
            mazeFound[y][x - 1].setWallRight(mazeActual[y][x - 1].getWallRight());
        }
        if (y < size - 1) {
            mazeFound[y + 1][x].setWallUp(mazeActual[y + 1][x].getWallUp());
        }
        if (y > 0) {
            mazeFound[y - 1][x].setWallDown(mazeActual[y - 1][x].getWallDown());
        }
    }
